...
//        }
        ...
        if (SDLControllerManager.isDeviceSDLJoystick(deviceId)) {
            ...
            if (event.getAction() == KeyEvent.ACTION_DOWN) {
                if (SDLControllerManager.onNativePadDown(deviceId, keyCode, event.getScanCode(), event.getRepeatCount()) == 0) {
                    return true;
                }
            } else if (event.getAction() == KeyEvent.ACTION_UP) {
                if (SDLControllerManager.onNativePadUp(deviceId, keyCode, event.getScanCode()) == 0) {
                    return true;
                }
            }
        }
